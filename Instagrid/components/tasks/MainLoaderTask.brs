...
sub GetContent()
    ...
    xfer.SetURL("https://testappjue.web.app/instagrid/instagridContent.json")
    ...
    if json <> invalid
        for each category in json
            ...
            if Type(value) = "roArray" ' if parsed key value having other objects in it
                if category = "instagridContent" ' only load diorama content
                    row = {}
                    row.title = category
                    row.children = []
...
function GetItemData(element as Object) as Object
    ...
    item.tUrl = element.turl
    item.description = element.description
    
    if element.content <> invalid
        item.length = element.content.duration
        item.url = element.content.videos[0].url
        item.streamFormat = element.content.videos[0].videoType
    end if
